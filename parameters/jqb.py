...
class Vacation(Document):
    ...
    @classmethod
    def fetch(cls):
        import parameters.fetchjq
        data=parameters.fetchjq.fetch()
    ...
    def update(cls,year,dates):
        ...
        for a in cls.objects(P.year>=year):
            if a.arrangement:
                ...
                cls.objects(year=a.year+1).upsert_one(base=[v[0],v[-2],v[-1]])
        return dates
    
    @classmethod
    def export(cls,basedate=None,years=8,fn = None):
        basedate=basedate or (dt.datetime.today()+dt.timedelta(days=5)).strftime('%Y%m%d')
        year=int(basedate[:4])
