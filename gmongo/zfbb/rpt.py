...
QuarterPattern = R/r'20\d{2}-[1234]'
...
QUARTER_SQL = ('select a."id",sum(b.vv),sum(b.vv2) from parameter a '
...
def export(qc):
    ...
    if QuarterPattern == qc:
        ...
        for i in (14, 2, 11):                                      # 分别获取当期、同比、环节数据
            for r in find(QUARTER_SQL, {'begin': months[i-2], 'end': months[i]}):
                data[r[0]].extend(r[1:])
